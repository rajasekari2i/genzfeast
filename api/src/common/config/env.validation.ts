...
const envSchema = z.object({
  ...
  // specs/014-msg91-sms-otp-mobile-verification — MSG91 SMS. Used for
  // registration-time mobile-number verification, and (revised) for
  // forgot-password OTP delivery when the requesting user's own Company has
  // `is_sms=true` (order-pickup OTP always stays on FCM regardless).
  // Deliberately optional, same reasoning as Firebase/Supabase/Razorpay: the
  // app boots without these; the relevant AuthService method throws/falls
  // back at request time instead (see msg91-sms.adapter.ts) — mobile
  // verification has no silent fallback (a send failure must be surfaced),
  // while forgot-password falls back to FCM push on an MSG91 failure,
  // matching how mobile-verification itself falls back to FCM when SMS
  // fails outright.
  MSG91_AUTH_KEY: z.string().optional(),
  MSG91_SENDER_ID: z.string().optional(),
  MSG91_DLT_ENTITY_ID: z.string().optional(),
  MSG91_TEMPLATE_ID_MOBILE_VERIFICATION: z.string().optional(),
  // Separate DLT-approved template — India's DLT regulation requires the
  // exact registered SMS text per distinct message, so this cannot reuse
  // MSG91_TEMPLATE_ID_MOBILE_VERIFICATION's template (different wording).

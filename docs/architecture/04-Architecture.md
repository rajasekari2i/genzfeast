...
| Push notifications | **Firebase Cloud Messaging (FCM) only** — no other Firebase product is used. Delivers password-reset and order-pickup OTP |
| SMS (mobile-number verification only) | **MSG91** — `specs/014`. Delivers ONLY the registration-time mobile-verification OTP; not a replacement for FCM above |
...
**One narrow exception**: registration-time mobile-number verification (`specs/014-msg91-sms-otp-mobile-verification`) delivers via SMS through **MSG91** instead of FCM — a prospective student has no app-installed/notification-permission state to rely on yet at that point, and this flow specifically needs to prove the *phone number itself* is reachable, which push cannot do. This is the only SMS-delivered flow in the platform; order/password-reset OTP stay on FCM as described above.
...
## 8. Notification Flow (OTP) — via FCM, plus one SMS exception
...
- **Mobile-number verification OTP** (numeric, new, `specs/014`): generated at registration time on request → stored in `mobile_verifications.otp_hash` + `otp_expires_at` (Postgres, pre-account — no `User` row exists yet) → sent via **MSG91** SMS, not FCM. Unlike the other two, this send has no fallback: a failure to send is surfaced as an error to the client, not silently swallowed, since the registration flow (when `MOBILE_VERIFICATION_REQUIRED=true`) depends on the student actually receiving this code. Requires its own TRAI DLT-registered SMS template, independent of Firebase's setup below.

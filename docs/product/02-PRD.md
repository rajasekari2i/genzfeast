...
|---|---|
| **Product** | GenzFeast |
| **Platforms** | React Native (Android + iOS), CSS NativeWind (Tailwind CSS compiler), Node.js backend with nestjs typescript & ORM prisma, Supabase (PostgreSQL + Storage), Firebase (Cloud Messaging only) |
| **Version** | 1.1 — updated for Supabase/PostgreSQL + stateless JWT auth |
...
- FR-1.3: System Admin can view/manage all Role platform-wide.
- FR-1.4: System Admin can view/manage all Category platform-wide.
- FR-1.5: System Admin can view/manage all department platform-wide.
...
### FR-4: Student/Teaching/Non_teahing Registration & Login
...
- FR-4.5: On successful login/registration, the backend issues a stateless JWT **access token** and a database-tracked **refresh token** (see Architecture §4). The client stores both securely and attaches the access token as `Authorization: Bearer <token>` on every subsequent request.
- FR-4.6: Logout revokes the current refresh token server-side; the corresponding access token remains valid only until its own short expiry elapses (accepted trade-off of statelessness).
...
### FR-10: Student Profile *(new)*
- FR-10.1: A Profile screen shows the logged-in student's Name, Username, Department, and Email.
- FR-10.2: **Username cannot be edited** from the Profile screen, or via the underlying API — it is display-only everywhere in this module.
- FR-10.3: An Edit action allows updating **Department** and **Email** only.
- FR-10.4: A separate Change Password action allows updating the password by providing the current password plus a new password (with retype confirmation) — distinct from the Forgot Password (OTP-based) flow in FR-5.
- FR-10.5: A successful password change revokes the student's other active sessions (refresh tokens), keeping only the current session logged in.

### FR-11: My Orders — History & Upcoming Orders *(new)*
- FR-11.1: A "My Orders" screen shows the student's own orders split into two views: **Upcoming** (not yet delivered: `payment_pending`, `order_placed`) and **History** (`delivered`, `payment_failed`, `cancelled`).
- FR-11.2: Tapping an order opens an Order Detail screen showing the itemized snapshot, total, status, and status-specific content (Pickup OTP if `order_placed`, delivered timestamp if `delivered`, Pay Again if `payment_failed`).
- FR-11.3: Order line items always reflect the price/name at the time the order was placed, not the product's current data.
- FR-11.4: The Pickup OTP is only exposed via the API while the order is `order_placed`; it is not returned once the order is `delivered`.

## 4. Non-Functional Requirements

| Category | Requirement |
|---|---|
| Multi-tenancy | All tenant-scoped data (users, products, orders, categories, departments) must be isolated by `company_id`; enforced both in the Node API's query layer and via PostgreSQL Row Level Security — a query/bug at either layer must never leak data across tenants. |
...
| Security | Passwords hashed (bcrypt/argon2, never stored plain) despite the raw `password` field name in the source model; OTPs are single-use and time-bound; authentication uses short-lived, stateless JWT access tokens plus revocable, database-tracked refresh tokens (see Architecture §4). |
...
| Auditability | All entities carry `created_by`, `updated_by`, `created_at`, `updated_at`, `is_deleted` for soft-delete and traceability; every mutation of a business-critical table (`companies`, `users`, `products`, `orders`, `deliveries`, etc.) is additionally recorded in `audit_logs` automatically via database triggers — see Data Model §10. |
| Notifications | OTPs and order status updates delivered via Firebase Cloud Messaging (FCM) — the **only** Firebase service used in this architecture. |
...
7. Should Name also be editable on the Profile screen, or is it intentionally locked like Username? (Current spec treats Name as read-only by default assumption — see Module 7.)
8. Should a `cancelled` order in History support any student-facing action (e.g., view refund status), or is it purely informational in V1?

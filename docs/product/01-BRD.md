...
| **Teaching (End User)** | Browses menu, orders, pays, picks up food using OTP |
| **Non_Teaching (End User)** | Browses menu, orders, pays, picks up food using OTP |
| **Engineering (Claude Code / dev team)** | Builds the system from these specs |

## 5. Scope — V1 (In Scope)

- Multi-tenant company (canteen) management by System Admin and create mater CRUD Category(Student, Teaching, Non_Teaching), CRUD Role(SYSTEM_ADMIN, COMPANY_ADMIN, STAFF, STUDENT, TEACHING, NON_TEACHING), CRUD Department(BE CS, BE ECE, BE EEE..etc) we need add the CRUD dynamically in the admin page. 
...
1. **[Updated]** The database is **Supabase (managed PostgreSQL)**, not Firebase/Firestore. **Firebase is retained only for Cloud Messaging (push notifications)** — no other Firebase product is used. *(Decided — see Architecture doc §2–§3.)*
...
| Row Level Security misconfiguration could allow cross-tenant data access | Two-layer defense: Node API scopes every query by `company_id` from the JWT *and* Postgres RLS policies independently re-check tenant scoping (Data Model §12) |
| Stateless access tokens cannot be revoked before natural expiry | Keep access-token lifetime short (15–60 min); revoke the paired refresh token immediately on logout/lock so the exposure window is bounded (Architecture §4) |

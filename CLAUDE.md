...
- Forgot-password uses a **separate** 6-character alphanumeric OTP (`reset_password_otp`). Delivery channel follows the requesting user's own Company's `is_sms` flag (same flag registration verification uses, below): SMS (MSG91) when `is_sms=true` (falling back to FCM push on an SMS send failure), FCM push when `is_sms=false`. Either way the push/SMS is a **visible** message the user reads and types the code from themselves — not a silent/data-only push with client-side auto-fill.
- Registration-time mobile-number verification (`mobile_verifications` table, new) is a separate, narrower flow with its own per-Company `is_sms`-driven SMS(MSG91)/push(FCM) routing — no longer the only SMS-delivered OTP in this codebase now that forgot-password shares the same routing, but it remains its own flow with its own DLT template (`MSG91_TEMPLATE_ID_MOBILE_VERIFICATION`, distinct from forgot-password's `MSG91_TEMPLATE_ID_PASSWORD_RESET` — India's DLT rules require one registered template per distinct message). It blocks registration completion only when `MOBILE_VERIFICATION_REQUIRED=true` (default `false`, until DLT template approval completes).

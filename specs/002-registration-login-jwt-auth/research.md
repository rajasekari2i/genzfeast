...
- **Decision**: On every login attempt, look up the account by `(company_id, username)` for a Company-scoped role, or by `username` alone (`company_id IS NULL`) for `system_admin` — matching the `users` table's own partial unique indexes (spec.md Clarifications 2026-09-06; see also §5a below on why `company_id` must be in the request at all). If it does not exist, or if it exists but `status = 'locked'` or `status = 'inactive'`... the two cases are handled differently on purpose:
...
### 5a. Why `company_id` must be part of the login request

- **Decision**: `LoginRequest` carries an optional `company_id` (required in practice for every role except `system_admin`), sourced by the client from its own per-tenant branded build config, never typed in by the user (contracts/openapi.yaml).
- **Rationale**: `001-company-role-user-setup`'s `username` uniqueness is `UNIQUE (company_id, lower(username))` — per Company, not global (`001` data-model.md) — and the same mobile number is explicitly allowed to hold independent accounts at two different Companies. A login lookup keyed on `username` alone is ambiguous whenever that happens, and could authenticate a request against the wrong Company's account entirely (a tenant-isolation defect, not just a UX one) if resolved incorrectly (e.g., "first match wins"). Since the mobile client is already built per-tenant (Architecture — Mobile client row), it always knows its own `company_id` at build/config time with no extra user-facing field needed.
- **Alternatives considered**: (a) Global username lookup, rejecting login outright on any cross-company collision — rejected because it would non-deterministically break login for any legitimate multi-Company user depending on registration order, and is a silent tenant-isolation risk if a future change ever picked a match instead of rejecting. (b) Making Username globally unique instead — rejected, contradicts `001`'s already-decided data model and the explicit BRD/CLAUDE.md allowance for the same mobile number across Companies.

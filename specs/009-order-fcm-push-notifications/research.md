...
## 7. Reaching a locked account for `003`'s forgot-password push (added during task planning, 2026-09-06)

- **Decision**: `device_registrations.refresh_token_id` is nullable. `003-forgot-password-otp-reset`'s `POST /auth/forgot-password/request` — which resolves `(company_id, username)` to an account with no session at all — may optionally submit an `fcm_token` in the same request; `DevicesService` exposes an internal `upsertUnauthenticated(userId, fcmToken)` method (not a public endpoint of its own) that `003`'s `AuthService.requestPasswordReset` calls directly, writing a `device_registrations` row with `refresh_token_id: NULL` under a trusted service-role context (the same pattern `006`'s payment webhook already uses to write `orders` outside any user's RLS-scoped session).
- **Rationale**: FR-008's own lock cascade (research.md §3) deletes every session-linked registration the instant an account locks — and a locked account is `003`'s primary use case. Without this second registration path, a locked account would have zero device registrations by the time it reaches Forgot Password, and since `003` already excludes ever returning the code via a REST response, there would be no channel left to deliver it at all. This was surfaced as a genuine cross-feature contradiction during `009`'s task planning and resolved by treating "register a device" as two related but independently-triggerable operations: the normal authenticated one (FR-001) and this one, rather than trying to make the lock cascade smarter (which would weaken FR-008/SC-004's privacy guarantee) or accepting the gap.
- **Alternatives considered**: Exempting account-lock from the cascade — rejected; it directly undermines the "zero cases of a stale device still receiving notifications after lock" bar FR-008/SC-004 was written for, for the sake of a scenario (password reset) that has its own, better-scoped fix available. Leaving the gap unresolved — rejected; it would leave `003`'s own SC-001 ("under 5 minutes, without contacting support") unmet for the exact case (a locked account) that flow exists to serve.

## Outstanding NEEDS CLARIFICATION

None. The spec's original draft shipped with zero clarification markers; one was added and resolved during `009`'s own task-planning pass (§7 above) once its interaction with `003`'s already-shipped design was traced through.
